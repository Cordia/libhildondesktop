                                         ...
                                         const gchar     *module_id)
{
  ...
  g_warning ("Plugin removed: %s", module_id);
  ...
  for (p = priv->plugins; p; p = p->next)
    {
      ...
      g_warning ("Plugin: %s", info->module_id);
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CONF_FILE:
      g_value_set_object (value, priv->config_file);
      break;
      ...
      for (p = all_plugins; p; p = p->next)
        {
          ...
          if (load)
            hd_plugin_manager_load_plugin (manager, plugin_id);
